...
const ShareSection: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    if (!section || !device || !qrCard || !leftHeadline || !rightHeadline || !subheadline || !cta || !bg) return;
  ...
  return (
    <section 
      ref={sectionRef}
      id="share"
      className="section-pinned bg-vaulta-dark z-40"
    >
      {/* Background Image */}
      <div 
        ref={bgRef}
      ...
      >
        <img 
          src="/images/hand_phone_closeup_bg.jpg" 
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center">
        ...
        <div 
          ...
          style={{ opacity: 0 }}
        >
          <DeviceMockup screen="share" />
        </div>

        {/* Floating QR Card */}
        <div 
          ref={qrCardRef}
          className="absolute left-[62vw] top-[56%] -translate-y-1/2 hidden lg:block"
          style={{ opacity: 0 }}
        >
          <div className="w-[min(18vw,260px)] p-5 rounded-[18px] bg-white/[0.08] border border-white/10 backdrop-blur-sm float-animation">
            <div className="aspect-square rounded-xl bg-white p-3 mb-3">
              <div className="w-full h-full bg-vaulta-dark rounded-lg flex items-center justify-center">
                <QrCode className="w-16 h-16 text-vaulta-text" />
              </div>
            </div>
            <p className="text-vaulta-muted text-xs text-center font-mono uppercase tracking-wider">
              Scan to verify
            </p>
          </div>
        </div>
